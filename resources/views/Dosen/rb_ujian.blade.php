                            ...
                            <table class="table" id="dataTable">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($arr as $key => $val)
                                        @php
                                            $st     = DB::table('trx_rb_ujian')->where('id_mhs', $val->id)->where('id_dospem', $id_dospem)->where('is_active', 1)->first();

                                            $cekset1 = DB::table('trx_setting_bimbingan')->where('id_mhs', $val->id)->where('id_dospej_1', $id_dospem)->where('is_active', 1)->first();
                                            $cekset2 = DB::table('trx_setting_bimbingan')->where('id_mhs', $val->id)->where('id_dospej_2', $id_dospem)->where('is_active', 1)->first();
...
<script>
    ...
    $(document).on("click", "[data-name='save_nilai']", function(e) {
        ...
        if(id_mhs === '' || nilai_sp1_1 === '' || nilai_sp1_2 === '' || nilai_sp1_3 === '' || nilai_sp1_4 === '' || nilai_sp1_5 === '' || nilai_sp1_6 === ''){
            Swal.fire({
                position: 'center',
                title: 'Form is empty!',
                icon: 'error',
            ...
            $.ajax({
                type: "POST",
                url: "{{route('add_nilai_rb_uji_dosen')}}",
                data: {
                    id_mhs: id_mhs,
                    judul: judul,
...
<script>
    $(document).on("click", "[data-name='shownilai']", function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "{{route('show_nilai_rb_uji_dosen')}}",
            data: {
                id_mhs: id_mhs,
            },
            cache: false,
